...
`sys` prints the report card, also shown at the top of every new terminal. Its last line is the sync summary: state and age of the last run, time to the next timer run, watcher and timer health, conflict count. A second line appears only for failures or skips. `sys -v` adds mounts, all sensors, failed unit names and the full `drive status`.
