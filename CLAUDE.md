...
Prefer, in order: Fedora or Terra RPM, vendor RPM repo, Copr, system flatpak, npm global or `go install` (CLI tools, land under mise's node and go), AppImage through Gear Lever. For a vendor repo, fetch the vendor's setup script, read it, and write the `.repo` file by hand with the same content; the script itself stays unexecuted. Record the result in `packages.md`.
...
`~/Developer` has five kind-directories and nothing else: `domain/` (one directory per domain for anything that ships to one), `tools/` (CLIs, libraries, templates), `ops/` (repos that run Kryft but aren't a product: `internal`, `ledger`, `sales`, `servers`), `lab/` (throwaway and learning), `uni/` (university coursework repos). Client deliverables such as PDFs and smoke-test bundles go under `ops/internal/clients/<client>/`, never loose at the top level. `.trash/` (the `rm` function's target, timestamped) and `.archive/` hold old copies; no `.env` is tracked in any repo. `cdd` (Kryft's Go TUI, `tools/cdd`) jumps to a project by recency and fuzzy filter; `cdd scan` reseeds its history after moving directories. `~/Downloads` is synced to Google Drive in full, OAuth client JSONs and backup-code files excepted by name, so anything dropped there is on Drive within a minute. Runtime state: `~/.local/state/drive/` (last run, muted skips), `/run/user/1000/sys-sample`, `~/.cache/rclone/bisync/`. Details in `rclone.md`. `~/.hidden` (stow package `nautilus`) keeps `Android`, `dotfiles`, `flutter` and `go` out of GNOME Files and GTK pickers without moving them; `ls` still shows them.
